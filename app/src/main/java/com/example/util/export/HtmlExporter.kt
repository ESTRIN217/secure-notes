...
class HtmlExporter : Exporter {
    ...
    private fun buildHtml(context: Context, notes: List<DecryptedNote>, embedder: HtmlMediaEmbedder): String {
        ...
        sb.append("body { font-family: sans-serif; padding: 5px; line-height: 1; background-color: #f9f9f9; color: #333; }")
        ...
        sb.append(".checklist { list-style: none; padding-left: 5px; margin: 4px 0; }")
        sb.append(".checklist li { margin: 4px 0; display: flex; align-items: center; gap: 6px; }")
        sb.append(".checklist li input[type=\"checkbox\"] { width: 16px; heigth: 16px; accent-color: #2196F3; cursor: pointer; }")
        sb.append(".callout { background: #e8f4fd; border-left: 4px solid #2196F3; padding: 8px 12px; border-radius: 4px; }")
        sb.append("a { color: #1565c0; } @media (prefers-color-scheme: dark) {body {background-color: #1a1a1a;color: #e0e0e0;}.note {background: #2a2a2a;border-color: #444;}h1 {color: #f0f0f0;}.meta {color: #aaa;}th {background: #333;}th, td {border-color: #555;}blockquote {background: #2a2a2a;border-left-color: #555;color: #ccc;}pre {background: #333;}.callout {background: #1a3a4a;}a {color: #64b5f6;}}")
        sb.append(".file-link a { font-weight: 600; } .video-link a { font-weight: 600; }")
        sb.append("details summary { cursor: pointer; font-weight: 600; }")
        sb.append(".page-link { color: #1565c0; font-weight: 600; }")
        sb.append("</style><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><title>Secure Notes</title>")
        sb.append("</head><body>")
