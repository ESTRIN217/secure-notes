...
class MarkdownConverter {
    ...
    private val markdownTransforms = listOf(
        ...
        TagTransform(Regex("<h4>(.*?)</h4>")) { "#### ${it.groupValues[1]}" },
        TagTransform(Regex("<h5>(.*?)</h5>")) { "##### ${it.groupValues[1]}" },
        TagTransform(Regex("<h6>(.*?)</h6>")) { "###### ${it.groupValues[1]}" },
        ...
        TagTransform(Regex("<mark>(.*?)</mark>")) { "==${it.groupValues[1]}==" },
        TagTransform(Regex("<small>(.*?)</small>")) { "<small>${it.groupValues[1]}</small>" },
